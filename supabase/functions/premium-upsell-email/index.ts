...
function buildHtml(firstName: string, streakDays: number): string {
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>What Premium unlocks for you</title><meta name="color-scheme" content="light dark"><style>@media (prefers-color-scheme:dark){img.cdl{display:none!important}img.cdd{display:inline!important}}</style></head>
...
Deno.serve(async (req: Request) => {
  ...
  if (body.direct_user_id) {
    ...
    const nameRaw   = user.display_name?.split(' ')?.[0] || 'there';
    const firstName = esc(nameRaw);
