...
const MILESTONES = [1, 3, 5, 7, 10, 14, 21, 30, 45, 60, 90, 100, 120, 150, 180, 200, 270, 365, 500, 548, 730, 821, 912, 1000, 1095, 1186, 1278, 1369, 1460, 1551, 1643, 1734, 1825, 1917, 2008, 2099, 2190, 2281, 2373, 2464, 2555, 2646, 2738, 2829, 2920, 3011, 3103, 3194, 3285, 3376, 3468, 3559, 3650];
...
export default function HomeScreen() {
  ...
  const doUndoRelapse = async () => {
    ...
    try {
      ...
      const { data: prefsRow } = await supabase.from('users').select('notif_milestone, notif_daily_streak, notif_weekly_summary, notif_milestone_approaching, notif_urge_prediction, notif_community').eq('id', user.id).maybeSingle();
      const prefs = {
        notif_milestone: prefsRow?.notif_milestone ?? DEFAULT_NOTIF_PREFS.notif_milestone,
        notif_daily_streak: prefsRow?.notif_daily_streak ?? DEFAULT_NOTIF_PREFS.notif_daily_streak,
        notif_weekly_summary: prefsRow?.notif_weekly_summary ?? DEFAULT_NOTIF_PREFS.notif_weekly_summary,
